...
module.exports.onLoad = () => {
    ...
    if (!fs.existsSync(lvb + "upt.mp4")) request("https://i.imgur.com/ArdDUY1.mp4").pipe(fs.createWriteStream(lvb + "upt.mp4"));
      }
module.exports.run = async function({ api, event, args, client }) {
  ...
  let seconds = Math.floor(time % 60);
    ...
    if(url.match(lvbang) == null) return api.sendMessage({body:`╭•┄┅════❁🌺❁════┅┄•╮\n•—»✨𝗨𝗣𝗧𝗜𝗠𝗘 𝗥𝗢𝗕𝗢𝗧✨«—•\n╰•┄┅════❁🌺❁════┅┄•╯\n\n𝗗𝗢𝗨𝗚𝗛 𝗧𝗜𝗠𝗥 𝗖𝗨𝗥𝗥𝗘𝗡𝗧𝗟𝗬 𝗢𝗡𝗟𝗜𝗡𝗘 𝗜𝗡 𝗧𝗢𝗧𝗔𝗟 ${hours} 𝗛𝗢𝗨𝗥𝗦 ${minutes} 𝗠𝗜𝗡𝗨𝗧𝗘 ${seconds} 𝗦𝗘𝗖𝗢𝗡𝗗 👾\n⋆✦⋆⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⋆✦⋆\nPlease enter/replit the url to post on Uptime Robot`, attachment: fs.createReadStream(__dirname + `/noprefix/upt.mp4`)}, event.threadID, event.messageID);
